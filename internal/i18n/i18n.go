...
var catalogZH = map[string]string{
	...
	"agentRuntime.incompatibleVersion":       "Codex CLI 版本过旧或无法识别，最低支持版本为 0.130.0。请在运行 Denova 的电脑上执行 codex update（npm 安装可执行 npm install -g @openai/codex@latest），更新后点击「检查连接」。",
	"agentRuntime.claudeIncompatibleVersion": "Claude Code 版本过旧或无法识别，最低支持版本为 2.1.259。请在运行 Denova 的电脑上执行 claude update（Homebrew 安装使用 brew upgrade claude-code），更新后点击「检查连接」。",
	...
	"api.lore.imageAgentNotCalled":               "图像 Agent 未调用图片生成工具，请检查图像 Agent 的模型与工具配置后重试。",
	"api.lore.imageAgentOutputMissing":           "图片生成工具已完成，但当前资料项没有关联到新图片。请查看本次图像 Agent 的运行记录。",
	"api.lore.imageAgentToolFailed":              "资料图片生成工具失败，请查看本次图像 Agent 的运行记录。",
	"api.lore.imageAgentToolFailedWithDetail":    "资料图片生成失败：{{detail}}",
...
var catalogEN = map[string]string{
	...
	"api.lore.imageAgentNotCalled":               "The Image Agent did not call the image generation tool. Check its model and tool settings, then retry.",
	"api.lore.imageAgentOutputMissing":           "The image generation tool completed, but no new image is attached to this lore item. Check the Image Agent run details.",
	"api.lore.imageAgentToolFailed":              "The lore image generation tool failed. Check the Image Agent run details.",
	"api.lore.imageAgentToolFailedWithDetail":    "Lore image generation failed: {{detail}}",
